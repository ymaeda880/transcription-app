...
# ------------------------------------------------------------
# ■ 目的：
#   Whisper API（whisper-1）で音声ファイルを文字起こしし、結果と概算料金を表示します。
#
# ■ 主な流れ：
...
#   6) Whisper API 呼び出し（リトライ付き）
#   7) 結果表示＋料金サマリー表
#   8) 🔽 追加：整形結果テキストの「.txt ダウンロード」「ワンクリックコピー」機能
#
# ■ 今回の変更点：
#   - サイドバーに料金を表示していた処理を削除
#   - 本文側に「料金の概要」を表形式でまとめるのみ
#   - ✅ ダウンロード＆コピーの2ボタンを結果テキストの直後に追加
# ============================================================
...
from config.config import get_openai_api_key, OPENAI_TRANSCRIBE_URL, WHISPER_PRICE_PER_MIN
...
DEFAULT_USDJPY = float(st.session_state.get("usd_jpy", 150.0))
...
if go:
    ...
    if audio_min is not None:
        usd = float(audio_min) * float(WHISPER_PRICE_PER_MIN)
